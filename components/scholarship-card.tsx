...
export function ScholarshipCard({ scholarship, onSelect }: ScholarshipCardProps) {
  ...
  return (
    <motion.button
      onClick={handleClick}
      className="group relative flex h-full min-w-0 flex-col overflow-hidden rounded-3xl border border-black/10 bg-white/80 text-left shadow-[0_18px_45px_-30px_rgba(15,20,25,0.35)] transition duration-500 hover:-translate-y-1 hover:border-luxe-gold/50 hover:shadow-[0_24px_65px_-32px_rgba(212,175,55,0.65)] focus:outline-none dark:border-white/10 dark:bg-white/5"
    ...
    >
      <div className="relative aspect-[3/4] w-full overflow-hidden">
        ...
        <div className="absolute bottom-4 left-4 right-4 space-y-2 text-sm text-white sm:space-y-3">
          <span className="inline-flex items-center rounded-full border border-white/30 bg-black/40 px-3 py-1 text-[10px] uppercase tracking-[0.45em]">
            {scholarship.levelTags.join(' • ')}
          </span>
          <h3 className="font-serif text-lg font-semibold leading-snug sm:text-xl">{scholarship.name}</h3>
      ...
      <div className="flex flex-1 flex-col gap-4 p-5 text-sm text-luxe-ash dark:text-luxe-ash/80">
        <p className="line-clamp-3 text-[0.95rem] text-luxe-ebony/80 dark:text-luxe-ivory/85">
